...
export function runBacktest(candles: Candle[], params: Params): BacktestResult {
  ...
  for (let i = 1; i < bars.length; i++) {
    ...
    if (position === 0) {
      ...
      // Short entry
      else if (prev.close > prev.bbUpper && prev.rsi > params.rsiOverbought) {
        position = -1;
        entryPrice = bar.open;
        entryAtr = prev.atr;
        positionValue = capital * params.positionSizePct;
      ...
      else if (bar.close > bar.bbMiddle && bar.rsi > params.rsiExitLong)
        exitPrice = bar.close;
      ...
      else if (bar.close < bar.bbMiddle && bar.rsi < params.rsiExitShort)
        exitPrice = bar.close;
...
export function monthlyPnlTable(
  ...
  // Group by year-month, take last value per month
